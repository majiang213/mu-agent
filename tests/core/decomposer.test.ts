...
describe('TaskDecomposer', () => {
  ...
  describe('Level 1 — sequential patterns', () => {
    it('decomposes Chinese sequential prompt (先...然后...)', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('先修复登录bug然后写测试');
    ...
    it('decomposes numbered list (1. ... 2. ...)', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('1. implement login 2. write tests 3. update docs');
      expect(r.level).toBe(1);
      expect(r.tasks.length).toBeGreaterThanOrEqual(2);
    });

    it('sequential tasks have chained dependencies', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('先实现功能然后写测试再更新文档');
    ...
    it('confidence >= 0.8 for sequential', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('先修复bug然后写测试');
      expect(r.confidence).toBeGreaterThanOrEqual(0.8);
    });
  });

  describe('Level 1 — parallel patterns', () => {
    it('decomposes comma-separated parallel tasks', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('实现功能、写测试、更新文档');
      expect(r.level).toBe(1);
      expect(r.tasks.every((t) => t.parallel === true)).toBe(true);
      expect(r.tasks.every((t) => t.dependencies.length === 0)).toBe(true);
    });

    it('parallel tasks share same parallelGroup', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('实现功能、写测试、更新文档');
  ...
  describe('Level 1 — mixed patterns', () => {
    it('decomposes mixed sequential+parallel prompt', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('先实现功能然后写测试和更新文档');
      expect(r.level).toBe(1);
  ...
  describe('Level 3 — fallback', () => {
    it('falls back to Level 3 for unstructured prompt', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('帮我优化一下这个项目');
      expect(r.level).toBe(3);
      expect(r.tasks).toHaveLength(1);
      expect(r.tasks[0]!.description).toBe('帮我优化一下这个项目');
    });

    it('Level 3 confidence is 1.0', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('do something');
      expect(r.confidence).toBe(1.0);
    });

    it('Level 3 task has empty dependencies', () => {
      const d = new TaskDecomposer();
      const r = d.decompose('fix the bug');
      expect(r.tasks[0]!.dependencies).toHaveLength(0);
    });
  });
