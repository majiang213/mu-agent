...
> **一个例外（不在默认路径上）：** `RESEARCH` state 暴露了可选的
> `webfetch` / `websearch` 工具，用于任务明确要求时在线查文档或报错。
> 仅 `RESEARCH` 有这两个工具 —— 其余所有 state（LOCATE、MODIFY、VERIFY 等）
> 无任何网络工具，LLM 调用始终留在本地。不使用 web 调研，则无任何数据离开你的机器。
